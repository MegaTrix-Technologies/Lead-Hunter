...
const SalesLedgerTab = ({ periodParams, onAddSale, onCollectPayment }) => {
      ...
      <div className="bg-[#0A0A0A] border border-[#222222] p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <form onSubmit={handleSearchSubmit} className="relative flex-1 max-w-md">
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#050505] border border-[#2B2B2B] focus:border-blue-500 focus:outline-none text-xs text-white transition-colors"
          />
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2 bg-[#050505] border border-[#2B2B2B] text-xs text-zinc-300 focus:border-blue-500 focus:outline-none cursor-pointer"
            >
          ...
          {onAddSale && (
            <button
              type="button"
              onClick={onAddSale}
              className="px-3 py-2 bg-emerald-950/60 hover:bg-emerald-900 text-emerald-300 border border-emerald-800/80 text-xs font-bold uppercase flex items-center gap-1.5 transition-colors cursor-pointer shrink-0"
                ...
                {sales.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-[#121212] transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-center whitespace-nowrap">
                        {!isFullyPaid && onCollectPayment ? (
                          <button
                            type="button"
                            onClick={() => onCollectPayment(item.id)}
                            className="px-2.5 py-1 bg-emerald-950 hover:bg-emerald-900 text-emerald-300 border border-emerald-700 text-[10px] font-bold uppercase transition-colors cursor-pointer flex items-center gap-1 mx-auto"
                            title="Collect milestone payment"
                          >
