                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3.5 max-w-[280px]">
                      ...
                      {/* Deal Won & Closed Interaction Attribution Banner */}
                      {lead.callStatus === 'sale' && (
                        <div className="mt-2 p-2 bg-emerald-950/40 border border-emerald-600/60 text-emerald-300 text-[10px] font-mono leading-relaxed shadow-[0_0_10px_rgba(16,185,129,0.15)]">
                          <div className="font-bold text-emerald-400 flex items-center gap-1">
                            <span>★ DEAL WON &amp; CLOSED</span>
                          </div>
                          <div className="text-zinc-300">
                            Lead Generated by: <span className="text-white font-semibold">{lead.generatedByName || lead.extractedByName || 'Sales Agent'}</span>
                            {lead.leadGeneratedAt && <span> on {new Date(lead.leadGeneratedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>}
                          </div>
                          {lead.closerName && (
                            <div className="text-emerald-400/90">
                              Closed by: <span className="text-white font-semibold">{lead.closerName}</span>
                              {lead.convertedAt && <span> on {new Date(lead.convertedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>}
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    {/* Contact Details (With DNC Opt-Out Lock) */}
                    <td className="p-3.5 space-y-1">
                      {lead.callStatus === 'Do Not Call' ? (
                        <div className="flex items-center gap-1.5 text-rose-400 font-mono text-[10px] font-bold bg-rose-950/40 border border-rose-800/60 px-2 py-1">
                          <ShieldAlert className="w-3 h-3 text-rose-500 shrink-0" />
                          <span>[DO NOT CALL - OPT-OUT]</span>
                        </div>
                      ) : (
                        <>
                    ...
                    <td className="p-3.5 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        {lead.callStatus === 'Do Not Call' ? (
                          <span 
                            title="DNC Opt-Out: Calling strictly prohibited" 
                            className="p-1.5 bg-rose-950/30 border border-rose-900/60 text-rose-500 cursor-not-allowed inline-flex items-center"
                          >
                          ...
                          <button
                            ...
                            title="View Converted Client Record"
                            className="p-1.5 bg-emerald-950/40 border border-emerald-700/60 text-emerald-400 hover:bg-emerald-900/50 transition-colors cursor-pointer"
                          >
